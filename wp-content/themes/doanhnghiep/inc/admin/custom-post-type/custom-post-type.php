...
*/
	...
	<?php
		...
		/* Phản hồi khách hàng */
	add_action( 'init', 'sunset_customer_custom_post_type' );
	add_filter('manage_partners_posts_columns','sunset_set_customer_columns');
	add_action('manage_partners_posts_custom_column','sunset_customer_custom_column',10,2);
	function sunset_customer_custom_post_type() {
		$labels = array(
			'name' 				=> 'Khách hàng',
	...
	function sunset_set_customer_columns($columns){
		$newColumns = array();
		$newColums['title'] = 'Title';
		$newColums['avatar'] = 'Avatar';
		return $newColums;
	...
	function sunset_customer_custom_column($column,$post_id){
		switch ($column) {
			case 'avatar':
			echo get_the_post_thumbnail();
			break;
